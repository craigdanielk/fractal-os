...
import { getEconomicsAction } from "@/app/actions";
...
export default function EconomicsDetailPage() {
  ...
  useEffect(() => {
    async function loadModel() {
      try {
        const models = await getEconomicsAction();
    ...
    <div className="p-6">
      ...
      <div className="max-w-4xl space-y-6">
        <div className="grid grid-cols-2 gap-4">
          ...
          <CollabField recordId={modelId} field="margin_targets" recordType="economics">
            <div className="mb-2">
              <label className="block text-sm font-medium mb-1">Margin Target (%)</label>
              <input
                type="number"
                defaultValue={model.margin_targets?.toString() || "0"}
                className="w-full p-2 border rounded bg-white dark:bg-neutral-900"
                readOnly
              />
            </div>
          ...
          <CollabField recordId={modelId} field="overhead_pct" recordType="economics">
            <div className="mb-2">
              <label className="block text-sm font-medium mb-1">Overhead (%)</label>
              <input
                type="number"
                defaultValue={model.overhead_pct?.toString() || "0"}
                className="w-full p-2 border rounded bg-white dark:bg-neutral-900"
                readOnly
              />
            </div>
